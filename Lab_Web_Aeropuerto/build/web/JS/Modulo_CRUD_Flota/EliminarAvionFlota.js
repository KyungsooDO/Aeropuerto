...
export function enviarFormularioEliminarAvion(tipo) {
    ...
    if (!tipo) {
        getJSONConfirmacion('ServletEliminarAvionFlota', datos, procesarRespuestas);
    } else {
        getJSONConfirmacion('ServletEliminarAvionFlota', datos, procesarRespuestasTodo);
    }
...
export function procesarRespuestas(datos) {
    ...
    if (datos !== "ERROR") {
        mensajeEliminacionAvion("SE ELIMINÓ LA FLOTA DE AVIÓN CON EXITO", "exito");
    } else {
        mensajeEliminacionAvion("NO SE PUDO ELIMINAR LA FLOTA DE AVION", "error");
    }
}
...
export function procesarRespuestasTodo(datos) {
    ...
    if (datos !== "ERROR") {
        mensajeEliminacionAvion("SE ELIMINARON LAS FLOTAS SELECIONADOS", "exito");
    } else {
        mensajeEliminacionAvion("NO SE PUDO ELIMINAR LAS FLOTAS", "error");
    }
}
