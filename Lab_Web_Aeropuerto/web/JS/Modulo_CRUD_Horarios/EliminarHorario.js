...
export function enviarFormularioEliminarAvion(tipo) {
    ...
    if (!tipo) {
        getJSONConfirmacion('ServletEliminarHorario', datos, procesarRespuestas);
    } else {
        getJSONConfirmacion('ServletEliminarHorario', datos, procesarRespuestasTodo);
    }
...
export function procesarRespuestas(datos) {
    ...
    if (datos !== "ERROR") {
        mensajeEliminacionAvion("SE ELIMINÓ EL HORARIO CON EXITO", "exito");
    } else {
        mensajeEliminacionAvion("NO SE PUDO ELIMINAR EL HORARIO", "error");
    }
}
...
export function procesarRespuestasTodo(datos) {
    ...
    if (datos !== "ERROR") {
        mensajeEliminacionAvion("SE ELIMINARON LOS HORARIOS SELECIONADOS", "exito");
    } else {
        mensajeEliminacionAvion("NO SE PUDO ELIMINAR LOS HORARIOS", "error");
    }
}
