...
export function enviarFormularioEliminarAvion(tipo) {
    ...
    if (!tipo) {
        getJSONConfirmacion('ServletEliminarVuelo', datos, procesarRespuestas);
    } else {
        getJSONConfirmacion('ServletEliminarVuelo', datos, procesarRespuestasTodo);
    }
...
export function procesarRespuestas(datos) {
    ...
    if (datos !== "ERROR") {
        mensajeEliminacionAvion("SE ELIMINÓ EL VUELO CON EXITO", "exito");
    } else {
        mensajeEliminacionAvion("NO SE PUDO ELIMINAR EL VUELO", "error");
    }
}
...
export function procesarRespuestasTodo(datos) {
    ...
    if (datos !== "ERROR") {
        mensajeEliminacionAvion("SE ELIMINARON LOS VUELOS SELECIONADOS", "exito");
    } else {
        mensajeEliminacionAvion("NO SE PUDO ELIMINAR LOS VUELOS", "error");
    }
}
